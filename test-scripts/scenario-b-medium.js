...
async function runScenario() {
  ...
  try {
    ...
    // 2. Add 100 items × 10 = 1,000 items
    console.log('2. Adding 1,000 items (10 × 100)...');
    const addStart = Date.now();
    for (let i = 0; i < 10; i++) {
      await page.click('#add-btn');
      await page.waitForTimeout(200); // Allow time for re-render
    }
    const addTime = Date.now() - addStart;
    metrics.actions.push({ action: 'add_1000_items', duration: addTime, iterations: 10 });
    await page.waitForTimeout(1000);
